...
df_users = pd.read_csv("./data/Active_Users.csv", index_col=0)
print(df_users.shape)


df_evals = pd.read_csv("./data/human_eval_analysis/attitude_tracking.csv")
...
df_posts = pd.read_csv("./data/direct_influence/posts_with_senti_and_topic.csv", parse_dates=["createdAt"])
df_comts = pd.read_csv("./data/direct_influence/comments_with_senti_and_topic.csv", delimiter=",",
                       parse_dates=["createdAt"]).dropna(subset=["CommenterId", "PostId"])
df_views = pd.read_csv("./data/postViews.csv", delimiter="|",
                       parse_dates=["createdAt"]).dropna(subset=["UserId", "PostId"])
df_likes = pd.read_csv("./data/Likes.csv", delimiter="|",
                       parse_dates=["createdAt"]).dropna(subset=["UserId", "PostId"])
df_repts = pd.read_csv("./data/ReportUsers.csv", delimiter="|", parse_dates=["createdAt"])
...
def compute_moment_pagerank(G, node_name, topic, polarity, pr_alpha,
    ...
    pr_value = nx.pagerank_scipy(moment_graph, alpha=pr_alpha, weight=None)
...
def short_compute_pagerank(n, t, topic, polar, pr_alpha):
    g, pr = compute_moment_pagerank(
        G, n, topic, polar, pr_alpha, timeline[t], attention_window)
    subg = g.copy()
    subg.remove_nodes_from([n for n in subg if n[0] == "v"])
    return {"full_graph": describe_graph(g), "exps_graph": describe_graph(subg), "pr": pr}


def do_params(pr_alpha):
    keyq = [(node_name, t, topic, polar, pr_alpha)
...
if __name__ == "__main__":
    ...
    if args.mode == "gen":
        cmds = [f"python compute_pr_no_beta.py -p {p}"
                for p in [0.9, 0.85, 0.7, 0.5, 0.3, 0.1]
                # for a in [0.5, 0.9, 0.7, 0.3, 0.1]
                # for b in [0.5, 0.9, 0.7, 0.3, 0.1]
                ]
        script = "\n".join(cmds)
        with open("./pr_script_no_ab.sh", "w") as fp:
            fp.write(script)
    elif args.mode == "pr":
        path = Path(f"res/pagerank_no_beta/{args.pr}.pkl")
        if not path.exists():
            print(f"output to {path}")
            d = do_params(pr_alpha=args.pr)
            with open(path, "wb") as fp:
                pickle.dump(d, fp)
    elif args.mode == "fig2":
        path = Path("res/pagerank_no_beta/fig2.pkl")
        if not path.exists():
            print(f"output to {path}")
            d = do_fig2()
            with open(path, "wb") as fp:
